...
function parseArgs(style_, content_, gpu_, preserveColor_, alpha_, styleInter_, mask_)
    opt = {}

    opt.style = style_
    opt.content = content_
    opt.vgg = 'models/vgg_normalised.t7'
    opt.decoder = 'models/decoder.t7'
    opt.gpu = gpu_
    opt.preserveColor = false
    opt.alpha = 1
    opt.styleInterpWeights = ''
    opt.mask = ''

    if (preserveColor_ ~= nil) then opt.preserveColor = preserveColor_ end
    if (alpha_ ~= nil) then opt.alpha = alpha_ end
    if (styleInter_ ~= nil) then opt.styleInterpWeights = styleInter_ end
    if (mask_ ~= nil) then opt.mask = mask_ end
